import type { Level } from '../types'

export const levels: Level[] = [
  {
    id: 1,
    name: '入门第一关',
    difficulty: 'easy',
    map: [
      ['#', '#', '#', '#', '#', '#'],
      ['#', ' ', ' ', ' ', ' ', '#'],
      ['#', ' ', 'P', 'B', ' ', '#'],
      ['#', ' ', ' ', 'T', ' ', '#'],
      ['#', ' ', ' ', ' ', ' ', '#'],
      ['#', '#', '#', '#', '#', '#']
    ],
    playerStart: { x: 2, y: 2 },
    boxes: [{ x: 3, y: 2 }],
    targets: [{ x: 3, y: 3 }],
    walls: [
      { x: 0, y: 0 }, { x: 1, y: 0 }, { x: 2, y: 0 }, { x: 3, y: 0 }, { x: 4, y: 0 }, { x: 5, y: 0 },
      { x: 0, y: 1 }, { x: 5, y: 1 },
      { x: 0, y: 2 }, { x: 5, y: 2 },
      { x: 0, y: 3 }, { x: 5, y: 3 },
      { x: 0, y: 4 }, { x: 5, y: 4 },
      { x: 0, y: 5 }, { x: 1, y: 5 }, { x: 2, y: 5 }, { x: 3, y: 5 }, { x: 4, y: 5 }, { x: 5, y: 5 }
    ]
  },
  {
    id: 2,
    name: '入门第二关',
    difficulty: 'easy',
    map: [
      ['#', '#', '#', '#', '#', '#', '#'],
      ['#', ' ', ' ', ' ', ' ', ' ', '#'],
      ['#', ' ', 'P', ' ', 'B', 'T', '#'],
      ['#', ' ', ' ', ' ', ' ', ' ', '#'],
      ['#', '#', '#', '#', '#', '#', '#']
    ],
    playerStart: { x: 2, y: 2 },
    boxes: [{ x: 4, y: 2 }],
    targets: [{ x: 5, y: 2 }],
    walls: [
      { x: 0, y: 0 }, { x: 1, y: 0 }, { x: 2, y: 0 }, { x: 3, y: 0 }, { x: 4, y: 0 }, { x: 5, y: 0 }, { x: 6, y: 0 },
      { x: 0, y: 1 }, { x: 6, y: 1 },
      { x: 0, y: 2 }, { x: 6, y: 2 },
      { x: 0, y: 3 }, { x: 6, y: 3 },
      { x: 0, y: 4 }, { x: 1, y: 4 }, { x: 2, y: 4 }, { x: 3, y: 4 }, { x: 4, y: 4 }, { x: 5, y: 4 }, { x: 6, y: 4 }
    ]
  },
  {
    id: 3,
    name: '中级第一关',
    difficulty: 'medium',
    map: [
      ['#', '#', '#', '#', '#', '#', '#'],
      ['#', ' ', ' ', 'T', ' ', ' ', '#'],
      ['#', ' ', ' ', ' ', 'B', ' ', '#'],
      ['#', ' ', 'P', ' ', ' ', ' ', '#'],
      ['#', ' ', ' ', 'T', ' ', ' ', '#'],
      ['#', ' ', ' ', 'B', ' ', ' ', '#'],
      ['#', '#', '#', '#', '#', '#', '#']
    ],
    playerStart: { x: 2, y: 3 },
    boxes: [{ x: 4, y: 2 }, { x: 3, y: 5 }],
    targets: [{ x: 3, y: 1 }, { x: 3, y: 4 }],
    walls: [
      { x: 0, y: 0 }, { x: 1, y: 0 }, { x: 2, y: 0 }, { x: 3, y: 0 }, { x: 4, y: 0 }, { x: 5, y: 0 }, { x: 6, y: 0 },
      { x: 0, y: 1 }, { x: 6, y: 1 },
      { x: 0, y: 2 }, { x: 6, y: 2 },
      { x: 0, y: 3 }, { x: 6, y: 3 },
      { x: 0, y: 4 }, { x: 6, y: 4 },
      { x: 0, y: 5 }, { x: 6, y: 5 },
      { x: 0, y: 6 }, { x: 1, y: 6 }, { x: 2, y: 6 }, { x: 3, y: 6 }, { x: 4, y: 6 }, { x: 5, y: 6 }, { x: 6, y: 6 }
    ]
  },
  {
    id: 4,
    name: '中级第二关',
    difficulty: 'medium',
    map: [
      ['#', '#', '#', '#', '#', '#', '#', '#'],
      ['#', ' ', ' ', ' ', ' ', ' ', ' ', '#'],
      ['#', ' ', 'P', ' ', 'B', ' ', ' ', '#'],
      ['#', ' ', ' ', '#', '#', '#', ' ', '#'],
      ['#', ' ', ' ', ' ', ' ', 'B', ' ', '#'],
      ['#', 'T', ' ', ' ', 'T', ' ', ' ', '#'],
      ['#', '#', '#', '#', '#', '#', '#', '#']
    ],
    playerStart: { x: 2, y: 2 },
    boxes: [{ x: 4, y: 2 }, { x: 5, y: 4 }],
    targets: [{ x: 1, y: 5 }, { x: 4, y: 5 }],
    walls: [
      { x: 0, y: 0 }, { x: 1, y: 0 }, { x: 2, y: 0 }, { x: 3, y: 0 }, { x: 4, y: 0 }, { x: 5, y: 0 }, { x: 6, y: 0 }, { x: 7, y: 0 },
      { x: 0, y: 1 }, { x: 7, y: 1 },
      { x: 0, y: 2 }, { x: 7, y: 2 },
      { x: 0, y: 3 }, { x: 3, y: 3 }, { x: 4, y: 3 }, { x: 5, y: 3 }, { x: 7, y: 3 },
      { x: 0, y: 4 }, { x: 7, y: 4 },
      { x: 0, y: 5 }, { x: 7, y: 5 },
      { x: 0, y: 6 }, { x: 1, y: 6 }, { x: 2, y: 6 }, { x: 3, y: 6 }, { x: 4, y: 6 }, { x: 5, y: 6 }, { x: 6, y: 6 }, { x: 7, y: 6 }
    ]
  },
  {
    id: 5,
    name: '高级挑战',
    difficulty: 'hard',
    map: [
      ['#', '#', '#', '#', '#', '#', '#', '#', '#'],
      ['#', ' ', ' ', ' ', '#', ' ', ' ', ' ', '#'],
      ['#', ' ', 'B', ' ', '#', ' ', 'T', ' ', '#'],
      ['#', ' ', ' ', 'P', ' ', 'B', ' ', ' ', '#'],
      ['#', ' ', 'T', ' ', '#', ' ', 'B', ' ', '#'],
      ['#', ' ', ' ', ' ', '#', ' ', 'T', ' ', '#'],
      ['#', '#', '#', '#', '#', '#', '#', '#', '#']
    ],
    playerStart: { x: 3, y: 3 },
    boxes: [{ x: 2, y: 2 }, { x: 5, y: 3 }, { x: 6, y: 4 }],
    targets: [{ x: 6, y: 2 }, { x: 2, y: 4 }, { x: 6, y: 5 }],
    walls: [
      { x: 0, y: 0 }, { x: 1, y: 0 }, { x: 2, y: 0 }, { x: 3, y: 0 }, { x: 4, y: 0 }, { x: 5, y: 0 }, { x: 6, y: 0 }, { x: 7, y: 0 }, { x: 8, y: 0 },
      { x: 0, y: 1 }, { x: 4, y: 1 }, { x: 8, y: 1 },
      { x: 0, y: 2 }, { x: 4, y: 2 }, { x: 8, y: 2 },
      { x: 0, y: 3 }, { x: 4, y: 3 }, { x: 8, y: 3 },
      { x: 0, y: 4 }, { x: 4, y: 4 }, { x: 8, y: 4 },
      { x: 0, y: 5 }, { x: 4, y: 5 }, { x: 8, y: 5 },
      { x: 0, y: 6 }, { x: 1, y: 6 }, { x: 2, y: 6 }, { x: 3, y: 6 }, { x: 4, y: 6 }, { x: 5, y: 6 }, { x: 6, y: 6 }, { x: 7, y: 6 }, { x: 8, y: 6 }
    ]
  }
]
