  ...
  template: `
    <div class="edit-password-container">
      <div class="header-section">
        ...
        <h1 class="page-title">编辑密码</h1>
        <p class="page-subtitle">修改密码记录的信息</p>
      </div>
      
      <div class="content-section" *ngIf="!isLoading; else loadingTemplate">
        <nz-card class="form-card" *ngIf="passwordEntry">
          <form [formGroup]="passwordForm" (ngSubmit)="onSubmit()" class="password-form">
            ...
            <div class="form-info">
              <div class="info-item">
                ...
                <span class="info-value">{{ formatDate(passwordEntry?.createdAt) }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">最后更新:</span>
                <span class="info-value">{{ formatDate(passwordEntry?.updatedAt) }}</span>
              </div>
              <div class="info-item" *ngIf="passwordEntry?.lastUsed">
                <span class="info-label">最后使用:</span>
                <span class="info-value">{{ formatDate(passwordEntry?.lastUsed) }}</span>
              </div>
            </div>
        ...
        <nz-card class="not-found-card" *ngIf="!passwordEntry">
          <div class="not-found-content">
            <span nz-icon nzType="exclamation-circle" nzTheme="outline" class="not-found-icon"></span>
            <h3>密码记录不存在</h3>
            <p>您访问的密码记录可能已被删除或不存在</p>
            <button 
              nz-button 
              nzType="primary"
              (click)="goBack()"
...
export class EditPasswordComponent implements OnInit {
  ...
  passwordEntry: PasswordEntry | undefined;
  passwordId: string = '';
  ...
  private loadPassword(): void {
    ...
    this.passwordStorageService.getPasswordById(this.passwordId).subscribe(password => {
  ...
  onSubmit(): void {
    if (this.passwordForm.invalid || !this.passwordEntry) {
      return;
    }
    ...
    this.passwordStorageService.updatePassword(this.passwordId, {
      ...
      next: () => {
        this.messageService.success('密码更新成功！');
        this.isSubmitting = false;
        this.router.navigate(['/']);
      },
      error: () => {
        this.messageService.error('更新失败，请重试');
        this.isSubmitting = false;
      }
    });
  }
